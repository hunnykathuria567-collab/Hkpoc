...
def get_company_signals():
    ...
    queries = [
        ...
        "\"Pvt Ltd\" factory expansion Haryana"
    ]
    all_snippets = []
    for q in queries:
        try:
    ...
    client = genai.Client()
    prompt = f"Identify top 15 companies from these snippets involved in >1Cr deals. Output JSON:\n" + "\n".join(all_snippets)
    try:
...
def deep_enrich_strike_team(company_name):
    ...
    try:
        res = requests.post("https://google.serper.dev/search", headers={'X-API-KEY': api_key, 'Content-Type': 'application/json'}, json={"q": q, "num": 10})
        snippets = "\n".join([r.get('snippet') for r in res.json().get('organic', [])])
        prompt = f"Find Name, Phone, and Email for CFO, Accountant, and Finance Lead for {company_name}. Look for 10-digit numbers. Results:\n{snippets}"
...
def send_excel_to_telegram(file_path):
    ...
    with open(file_path, 'rb') as f:
        requests.post(url, data={'chat_id': chat_id, 'caption': "📊 Master Lead Database (Limit 1,000)"}, files={'document': f})
